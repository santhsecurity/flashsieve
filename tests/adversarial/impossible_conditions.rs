use flashsieve::{BlockIndexBuilder, BlockIndex, ByteFilter, NgramBloom, NgramFilter, MmapBlockIndex};
use std::fs::File;
...
fn test_bloom_filter_1m_patterns() {
    ...
    let mut prng_state: u32 = 0x12345678;
    ...
    let res = bloom.maybe_contains(0, 0);
    assert!(res || !res);
}
...
fn test_bloom_filter_fpr_10m() {
    ...
    let mut prng_state: u32 = 0x87654321;
    let mut inserted = 0;
    while inserted < 500 {
        prng_state ^= prng_state << 13;
        prng_state ^= prng_state >> 17;
...
fn test_pattern_at_every_possible_offset() {
    ...
    for offset in 0..block_size {
        ...
        assert!(!candidates.is_empty(), "Pattern at offset {} not found", offset);
        
        // Ensure the candidate range actually covers the pattern
        let covers = candidates.iter().any(|c| {
            c.offset <= offset && (c.offset + c.length) >= (offset + pattern.len())
        });
        assert!(covers, "Candidates {:?} do not cover offset {}", candidates, offset);
    }
}
...
fn test_pattern_exactly_block_size() {
    ...
    assert!(!candidates.is_empty(), "Pattern exactly block size not found");
}
...
fn test_corrupted_bloom_bits_recovery() {
    ...
    if let Err(flashsieve::Error::ChecksumMismatch { expected: _, computed: _ }) = result {
        // Success
    } else {
...
fn test_data_all_zeros() {
    ...
    let index = BlockIndexBuilder::new().block_size(256).bloom_bits(1024).build(&data).unwrap();
...
fn test_alternating_bytes() {
    ...
    for i in 0..1024 { data[i] = if i % 2 == 0 { 0xAA } else { 0x55 }; }
    let index = BlockIndexBuilder::new().block_size(256).bloom_bits(1024).build(&data).unwrap();
...
fn test_streaming_build_empty() {
    ...
    let index = BlockIndexBuilder::new().block_size(256).bloom_bits(1024).build_streaming(iter).unwrap();
    assert_eq!(index.block_count(), 0);
}
...
fn test_streaming_build_wrong_size() {
    ...
    let result = BlockIndexBuilder::new().block_size(256).bloom_bits(1024).build_streaming(iter);
...
fn test_bloom_target_fpr_nan() {
    let res = NgramBloom::with_target_fpr(std::f64::NAN, 1000);
    assert!(res.is_ok());
}

#[test]
fn test_zero_bloom_bits() {
    let result = BlockIndexBuilder::new().block_size(256).bloom_bits(0).build(&[0u8; 256]);
    assert!(result.is_err());
}

#[test]
fn test_unaligned_block_size() {
    let result = BlockIndexBuilder::new().block_size(300).bloom_bits(1024).build(&[0u8; 300]);
    assert!(result.is_err());
}

#[test]
fn test_block_size_too_small() {
    let result = BlockIndexBuilder::new().block_size(128).bloom_bits(1024).build(&[0u8; 128]);
    assert!(result.is_err());
}
...
fn test_concurrent_query_same_index() {
    let index = Arc::new(BlockIndexBuilder::new().block_size(256).bloom_bits(1024).build(&[0u8; 1024]).unwrap());
    let mut handles = vec![];
    for _ in 0..8 {
