...
fn zero_false_negatives_across_scales() {
    ...
    for &n in &[1usize, 10, 100, 1000, 10000, 100000] {
        ...
        for p in &patterns {
            insert_pattern_ngram(&mut bloom, p);
        }
        for p in &patterns {
            assert!(
                bloom.maybe_contains_pattern(p),
                "FALSE NEGATIVE at scale N={n} for pattern {p:?}"
            );
        }
    }
    for &n in &[100usize, 1000, 10000] {
        ...
        for p in &patterns {
            insert_pattern_blocked(&mut bloom, p);
        }
        for p in &patterns {
            assert!(
                blocked_maybe_contains_pattern(&bloom, p),
                "FALSE NEGATIVE in BlockedNgramBloom at scale N={n} for pattern {p:?}"
            );
        }
    }
}
...
fn false_positive_rate_matches_theory() {
    ...
    for _ in 0..TRIALS {
        ...
        if bloom.maybe_contains_bloom(a, b) {
            false_positives += 1;
        }
    }
    ...
    assert!(
        diff < 0.02,
        "FP rate mismatch: measured={measured:.4}, theoretical={theoretical:.4}, diff={diff:.4}"
    );
}
...
fn hash_independence_verified() {
    ...
    let (mut idx0, mut idx1, mut idx2) = (
        vec![0usize; SAMPLES],
        vec![0usize; SAMPLES],
        vec![0usize; SAMPLES],
    );
    ...
    for probe in [&idx0, &idx1, &idx2] {
        ...
        assert!(
            counts.iter().copied().min().unwrap() > 0,
            "uniformity failure: empty bucket"
        );
        assert!(
            counts.iter().copied().max().unwrap() < (expected * 3.0) as usize,
    ...
    fn pearson(x: &[usize], y: &[usize]) -> f64 {
        let n = x.len() as f64;
        let (mx, my) = (
            x.iter().sum::<usize>() as f64 / n,
            y.iter().sum::<usize>() as f64 / n,
        );
        let num: f64 = x
            .iter()
            .zip(y)
    ...
    assert!(
        pearson(&idx0, &idx1).abs() < 0.01,
        "correlation idx0-idx1 too high"
    );
    assert!(
        pearson(&idx0, &idx2).abs() < 0.01,
        "correlation idx0-idx2 too high"
    );
    assert!(
        pearson(&idx1, &idx2).abs() < 0.01,
        "correlation idx1-idx2 too high"
    );
}
...
fn capacity_overflow_never_creates_false_negatives() {
    ...
    for &(a, b) in &inserted {
        assert!(bloom.maybe_contains(a, b), "false negative after overflow");
    }
    ...
    assert!(
        rate > 0.5,
        "expected severe FP degradation after overflow, got {rate}"
    );
}
...
fn serialization_roundtrip_zero_false_negatives() {
    ...
    file.write_all(&[if exact_pairs.is_some() { 1u8 } else { 0u8 }])
        .unwrap();
    ...
    let read_u64 = |slice: &mut &[u8]| -> u64 {
        let (bytes, tail) = slice.split_at(8);
        *slice = tail;
    ...
    for _ in 0..read_bits_len {
        read_bits.push(read_u64(&mut rest));
    }
    let has_exact = rest[0] == 1;
    rest = &rest[1..];
    let read_exact_pairs = if has_exact {
        let mut ep = Box::new([0u64; 1024]);
        for i in 0..1024 {
            ep[i] = read_u64(&mut rest);
        }
        Some(ep)
    } else {
        None
    ...
    let reconstructed =
        NgramBloom::from_serialized_parts(read_num_bits, read_bits, read_exact_pairs).unwrap();
    // Verify exact-pairs were restored by checking a known-absent n-gram returns false.
    let inserted_ngrams: HashSet<(u8, u8)> = patterns
        .iter()
        .flat_map(|p| p.windows(2).map(|w| (w[0], w[1])))
        .collect();
    let absent = (0u8..=255)
        .flat_map(|a| (0u8..=255).map(move |b| (a, b)))
        .find(|p| !inserted_ngrams.contains(p));
    if let Some((a, b)) = absent {
        assert!(
            !reconstructed.maybe_contains(a, b),
            "exact-pairs likely missing: true for uninserted ({a}, {b})"
        );
    }
    for p in &patterns {
        assert!(
            reconstructed.maybe_contains_pattern(p),
            "false negative after roundtrip for pattern {p:?}"
        );
    }
    ...
    for p in &small_patterns {
        insert_pattern_ngram(&mut small, p);
    }
    let (nb, sbits) = small.raw_parts();
    let raw_reconstructed = NgramBloom::from_raw_parts(nb, sbits.to_vec()).unwrap();
    for p in &small_patterns {
        assert!(
            raw_reconstructed.maybe_contains_pattern(p),
            "false negative after raw roundtrip for pattern {p:?}"
        );
    }
}
...
fn concurrent_queries_match_single_threaded() {
    ...
    for _ in 0..16 {
        ...
        handles.push(thread::spawn(move || {
            ...
            assert_eq!(
                actual, e,
                "concurrent query results diverged from single-threaded"
            );
        }));
    }
    for h in handles {
...
fn empty_bloom_rejects_everything() {
    ...
    for a in 0u8..=255 {
        for b in 0u8..=255 {
            assert!(
                ...
                "empty NgramBloom returned true for ({a}, {b})"
            );
        }
    }
    ...
    for a in 0u8..=255 {
        for b in 0u8..=255 {
            assert!(
                !blocked.maybe_contains(a, b),
                "empty BlockedNgramBloom returned true for ({a}, {b})"
            );
        }
    }
    ...
    assert!(
        bloom.maybe_contains_all(&[]),
        "empty maybe_contains_all should be vacuously true"
    );
    assert!(
        !bloom.maybe_contains_any(&[]),
        "empty maybe_contains_any should be false"
    );
}
...
fn adversarial_collisions_no_false_negatives() {
    ...
    for g in groups.values() {
        if g.len() > max_group.len() {
            max_group.clone_from(g);
        }
    }
    ...
    for &(a, b) in &max_group {
        assert!(
            bloom.maybe_contains(a, b),
            "false negative for adversarial collision pair ({a}, {b})"
        );
    }
    ...
    for a in 0u8..=255 {
        for b in 0u8..=255 {
            exhaustive.insert_ngram(a, b);
        }
    }
    for a in 0u8..=255 {
        for b in 0u8..=255 {
            assert!(
                exhaustive.maybe_contains(a, b),
                "false negative in exhaustive adversarial load for ({a}, {b})"
            );
        }
    }
}
